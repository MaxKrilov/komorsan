<!-- =========================================================================================
  File Name: AddNewDataMonitoringSidebar.vue
...
<template>
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    <div class="mt-6 flex items-center justify-between px-6">
        <h4>{{ Object.entries(this.data).length === 0 ? "Добавить новый" : "Обновить имеющийся" }} прибор</h4>
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <!-- NAME -->
        <vs-input label="Имя прибора" v-model="dataName" class="mt-5 w-full" name="item-name" v-validate="'required'" />
        <span class="text-danger text-sm" v-show="errors.has('item-name')">{{ errors.first('item-name') }}</span>

        <!-- CATEGORY -->
        <vs-select v-model="dataCategory" label="Описание" class="mt-5 w-full" name="item-category" v-validate="'required'">
        ...
        <!-- ORDER STATUS -->
        <vs-select v-model="dataOrder_status" label="Статус выхода на связь" class="mt-5 w-full">
...
export default {
  ...
  data() {
    return {
      ...
      category_choices: [
        {text:'Ти прибора',value:'ти прибора'},
        {text:'Не выходли на связь',value:'Не выходли на связь'},
        {text:'Авария',value:'авария'}
      ],

      order_status_choices: [
        {text:'На связи',value:'на связи'},
        {text:'Не выходли на связь',value:'Не выходли на связь'},
        {text:'Авария',value:'авария'}
      ],
