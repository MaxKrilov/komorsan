  ...
  export default {// locale event type
     ...
     changeLanguageTime(dateAndTimeNew) {
      if (dateAndTimeNew === '') {
        ...
        var dateOption = new Date( Date.parse( dateAndTimeNew ) );  // превращаем строку с временем в объект Date

        if ( window.language === 'EN' ) {
          dateOption = dateOption.toLocaleString('en', optionsNowDate);  // локализуем дату
          dateOption = dateOption.split(',');  // разбиваем строку и превращаем в массив
          dateOption = dateOption[0] + ',' + dateOption[1]+ '<br>' + dateOption[2];  // собираем строку в нужном нам виде
          return dateOption;
        } else if ( window.language === 'RU' ) {
          dateOption = dateOption.toLocaleString('ru', optionsNowDate);  // локализуем дату
          dateOption = dateOption.split(' г.,')[0] + '<br>' + dateOption.split(' г.,')[1];  // собираем строку в нужном нам виде
          return dateOption;
        }
      }
    },
